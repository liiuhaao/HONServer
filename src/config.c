...
void parse_config(const char *json_str, struct HONConfig *config)
{
    sscanf(json_str, "{\n\"dropRate\": %d,\n\"dataNum\": %d,\n\"parityNum\": %d,\n\"rxNum\": %d,\n\"encodeTimeout\": %ld,\n\"decodeTimeout\": %ld,\n\"rxTimeout\": %ld,\n\"ackTimeout\": %ld,\n\"primaryProbability\": %d,\n\"mode\": %d\n",
           &config->drop_rate, &config->data_num, &config->parity_num, &config->rx_num, &config->encode_timeout, &config->decode_timeout, &config->rx_timeout, &config->ack_timeout, &config->primary_probability, &config->mode);
